...
function Overview(){
  ...
  const upstreamStale = !!sync && (sync?.sourceCounts?.agents===0 && sync?.sourceCounts?.skills===0 && (sync?.effectiveCounts?.agents>0 || sync?.effectiveCounts?.skills>0));
  return <div className='space-y-4'>
    {upstreamStale && <div className='card border border-red-500/40 bg-red-900/20'><div className='font-semibold text-red-300'>Upstream Sync Alarm</div><div className='text-sm text-red-200'>Upstream source count 0 ama efektif agent/skill var. Endpoint mapping veya payload parse kontrol edilmeli.</div><div className='mt-2 flex gap-2'><button className='bg-red-800 px-2 py-1 rounded text-xs' onClick={async()=>{await api.post('/system/sync-now'); const s=await api.get('/system/sync-status'); setSync(s.data);}}>Force sync</button><button className='bg-zinc-800 px-2 py-1 rounded text-xs' onClick={()=>window.open(`${import.meta.env.VITE_API_URL||'/api'}/system/upstream/probe`,'_blank')}>Open probe</button></div></div>}
    ...
    <div className='card'><div>Agent uptime: {Math.floor(data.agent.uptimeSec)}s | version: {data.agent.version}</div><div className='text-xs text-zinc-400 mt-1'>Sync: {sync?.lastOkAt?new Date(sync.lastOkAt).toLocaleTimeString():'-'} | src agents:{sync?.sourceCounts?.agents??0} skills:{sync?.sourceCounts?.skills??0} tasks:{sync?.sourceCounts?.tasks??0}</div><div className='text-xs text-zinc-500'>effective agents:{sync?.effectiveCounts?.agents??0} skills:{sync?.effectiveCounts?.skills??0} tasks:{sync?.effectiveCounts?.tasks??0}</div><div className='text-xs text-zinc-500 break-all'>{sync?.upstreamRawKinds?JSON.stringify(sync.upstreamRawKinds):''}</div><div className='text-xs text-amber-400'>{sync?.lastError||''}</div><button className='mt-2 bg-zinc-800 px-2 py-1 rounded text-xs' onClick={async()=>{await api.post('/system/sync-now'); const s=await api.get('/system/sync-status'); setSync(s.data);}}>Sync now</button></div>
...
function Skills(){
  ...
  return <div className='space-y-3'>
    ...
    <div className='card'><table className='w-full text-sm'><thead><tr><th>ID</th><th>Ad</th><th>Açıklama</th><th>Atanan Agent</th><th>Kaynak</th><th/></tr></thead><tbody>{skills.map(s=><tr key={s.id}><td>{s.id}</td><td>{s.name}</td><td>{s.description}</td><td>{s.assignedAgentId||'-'}</td><td><span className='text-xs px-2 py-1 rounded bg-zinc-800'>{s.source||'local'}</span></td><td><button onClick={()=>setDetail({...s})}>Detay</button> | <button onClick={()=>api.post(`/skills/${s.id}/assign`,{agentId}).then(load)}>Ata</button> | <button onClick={()=>api.delete(`/skills/${s.id}`).then(load)}>Sil</button></td></tr>)}</tbody></table></div>
...
function Files(){
  const [path,setPath]=useState('projects/dashboard'); const [items,setItems]=useState([]); const [content,setContent]=useState(''); const [selected,setSelected]=useState('');
  const load=async(p=path)=>{setPath(p); setItems((await api.get('/files/list',{params:{path:p}})).data)}; useEffect(()=>{load('projects/dashboard')},[]);
