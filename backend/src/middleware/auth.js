...
export function authOrApiKeyMiddleware(req, res, next) {
  ...
  if (
    req.method === 'GET' &&
    ['/agents', '/skills', '/overview', '/system/stats', '/system/upstream', '/system/upstream/probe', '/system/sync-status', '/analytics', '/logs/export'].some((p) => req.path === p || req.path.startsWith(`${p}/`))
  ) {
